...
class DjiBridgeDrone(Drone):
    ...
    def __init__(
        self,
        bridge_url: str = "ws://192.168.1.100:8765",
        max_retries: int = 3,
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(self._max_retries):
            try:
                self._ws = await asyncio.wait_for(
                    websockets.connect(self._bridge_url, ping_interval=30),
                    timeout=10.0,
                )
                ...
                self._reconnect_attempts = 0
                log.info(
                    "Connected to DJI bridge at %s (attempt %d)",
                    self._bridge_url,
                    attempt + 1,
                )
                return
                ...
                last_error = e
                log.warning(
                    "DJI bridge connection attempt %d/%d failed: %s",
                    attempt + 1,
                    self._max_retries,
                    e,
                )
        ...
        raise ConnectionError(
            f"Failed to connect to DJI bridge at {self._bridge_url} "
