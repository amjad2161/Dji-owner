...
class MavlinkDrone(Drone):
    ...
    async def telemetry_stream(self) -> AsyncIterator[Telemetry]:
        """Stream telemetry at a reasonable rate (2 Hz).
        
        Uses cached values from background task for efficiency,
        with periodic refresh from telemetry system.
        """
        while self._connected:
            try:
                # Use cached position if available, otherwise fetch fresh
                if self._cache.position:
                    pos = self._cache.position
                else:
                    async for p in self._system.telemetry.position():
                        pos = p
                        break

                # Fetch other telemetry
                    ...
                    async for att in self._system.telemetry.attitude_euler():
                        async for gps in self._system.telemetry.gps_info():
                            yield Telemetry(
                                ...
                                gps_satellites=gps.num_satellites,  # Fixed: now properly getting satellites
                                gimbal_pitch_deg=0.0,
                                flight_mode=FlightMode.MISSION,
                            )
                            await asyncio.sleep(0.5)  # 2 Hz rate
                            break  # Break out of battery loop
                        break  # Break out of attitude loop
                    break  # Break out of GPS loop
