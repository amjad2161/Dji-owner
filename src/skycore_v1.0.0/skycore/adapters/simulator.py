...
class SimulatorDrone(Drone):
    ...
    def _step(self) -> None:
        ...
        # Battery model: realistic LiPo curve for 3S DJI battery
        # Voltage is non-linear: steep at ends, flat in the middle
        # 100% = 12.6V, 0% = 9.0V (cutoff)
        # Using a sigmoid-like approximation for realistic behavior
        if self.battery_percent >= 80:
            # Steep region: 80-100%
            self.battery_voltage = 11.1 + (self.battery_percent - 80) * 0.075
        elif self.battery_percent >= 20:
            # Flat region: 20-80% (most of flight time)
            self.battery_voltage = 10.8 + (self.battery_percent - 20) * 0.025
        else:
            # Steep region: 0-20%
            self.battery_voltage = 9.0 + (self.battery_percent * 0.09)
