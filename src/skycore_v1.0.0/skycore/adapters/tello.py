...
class TelloDrone(Drone):
    ...
    async def set_velocity(self, vx: float, vy: float, vz: float, yaw_rate: float = 0.0) -> None:
        loop = asyncio.get_running_loop()
        # Clamp all values to valid Tello RC range [-100, 100]
        def clamp(value: float) -> int:
            return int(max(-100, min(100, value)))
        
        # Tello rc command takes: left-right, fwd-back, up-down, yaw
        # Mapping: vx=forward, vy=left/right, vz=up/down (inverted), yaw_rate
        # Values should be in range [-100, 100] - multiply by 100
        await loop.run_in_executor(
            None,
            self._tello.send_rc_control,
            clamp(vy * 100),    # left-right
            clamp(vx * 100),    # fwd-back
            clamp(-vz * 100),  # up-down (Tello uses negative for up)
            clamp(yaw_rate),
        )
